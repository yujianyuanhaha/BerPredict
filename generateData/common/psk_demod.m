function [minBER] = psk_demod(sig, bitsPerSym, sampsPerSym, eBW, bits, trim)
%function [minBER] = psk_demod(sig, bitsPerSym, sampsPerSym, eBW, bits, trim)
%
% This function demodulates a PSK symbol stream and returns the bit error
% rate
%
% Inputs
%   sig - the received signal with pulse modulated PSK symbols
%   bitsPerSym - bits per symbol of PSK M = 2^bitsPerSym
%   sampsPerSym - samples per symbol of pulse shape used in PSK
...
% symbol timing
    Nsyms = floor(length(sig)/sampsPerSym);
    sig = sig(1:Nsyms*sampsPerSym); sig = sig(:).';
%     sig = [sig zeros(1,mod(length(sig),sampsPerSym))];
    avg_sym_mag = sum(reshape(abs(sig),sampsPerSym,[]),2);
