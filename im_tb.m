%  This script is the software testbed for simulating the 
% various IM techniques and interference types.  Details are given in the
% SVD document.  
% Four IM techniques are available (Frequency domain
% cancellation, Time-domain notching, DSSS-based transversal filtering, and
% fraction Fourier Transform)
% 
% Five interference types are available: NONE, CW, Chirp, filtered noise,
% and modulated interference.
%
% BER will be plotted vs Jammer-to-Signal Ratio
%
% Developed by Virginia Tech with the support of GIRD for the RRPUCS
% project
...
% rand('seed',0);
% randn('seed',0);
addpath('C:\Users\Yue Xu\Desktop\Latest/common');
addpath('C:\Users\Yue Xu\Desktop\Latest/NBI/NBI - DSSS');
addpath('C:\Users\Yue Xu\Desktop\Latest/NBI/NBI - Frequency Domain Nonlinear');
addpath('C:\Users\Yue Xu\Desktop\Latest/NBI/NBI - Frequency Domain Nonlinear\FilterBank');
addpath('C:\Users\Yue Xu\Desktop\Latest/NBI/NBI - Time Domain Linear');
...
NumIter = 2;    % number of iterations to run per JtoS value; 
                % randomizes impact of frequency offset

%===============Windowing Setting======================%
isWin = 1;      % windowing option 1 == use window; 0 == don't use window
...
if isWin==1
    ...
    tag = strcat(ModTag,intParams.type,',EbNo = ',num2str(EbNo),'dB',', No window');
end

% want window size to be a power of 2. Change to closest power of 2
windowSize = correctWinSize(windowSize);
...
%=============Mitigation Part ========================%
        ...
        if(strcmp(method,'DsssNbiCancel'))    
                if isWin
                    ...
                    x_end1 = r;
                    [sig,EstOffset] = time_align_DSSS(x_end1, beta, span, sps, Ns, pn);
                    [sig,EstFost] = frequencyCompDsss(sig,sps,pn,fs,checkUpbd, precision, Ns, N,bitsPerSym);
                    x_end1 = sig;
                    x_end2 = downsample(x_end1,sps);
                    ...
                    [sig,EstFost] = frequencyCompDsss(sig,sps,pn,fs,checkUpbd, precision, Ns, N,bitsPerSym);
                    x_end4 = sig;
                    x_end5 = downsample(x_end4,sps);
                    [Rxx, p] = CorrleationMatrixCalc(x_end5, FilterLength);
                    x_end3 = DsssNbiCancel(x_end5, Rxx, p);
        ...
        if(strcmp(method,'FFT-Thresh') && isDSSS == 1)    
            %==================Frequency offset mitigate========%       
                if isWin
                    %x_end3 = windowProcessDsss(r,windowSize, qq ,sps,FilterLength,  beta, span, Ns, pn, To, N, fs,checkUpbd, precision);
                    [sig,EstOffset] = time_align_DSSS(r, beta, span, sps, Ns, pn);
                    [sig,EstFost] = frequencyCompDsss(sig,sps,pn,fs,checkUpbd, precision, Ns, N,bitsPerSym);
                    x_end1 = sig;
                    x_end2 = downsample(x_end1,sps);
